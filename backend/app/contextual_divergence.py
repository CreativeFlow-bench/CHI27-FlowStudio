...
def _llm_enrich_fragments(
    *,
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "输出6-10个中文发散关键词短语（每个2-8个字）。"
                "要求：只基于候选概念联想，不编造无关概念；短语要具体、可建模"
                "（涉及材质、纹理、形状、装饰、结构）；不要与已有建议重复。"
    ...
    try:
        content = _llm_chat_completion(messages, timeout_sec=timeout_sec, temperature=0.9)
    ...
    return fragments, {"status": "ok" if fragments else "empty", "count": len(fragments)}
