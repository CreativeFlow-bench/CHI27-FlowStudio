...
def _qwen_cross_domain_response(
    *,
    ...
    if not endpoint:
        return None
    divergence_meta = request.metadata if isinstance(request.metadata, dict) else {}
    temperature = divergence_meta.get("temperature")
    try:
        temperature = float(temperature) if temperature is not None else 0.2
    except (TypeError, ValueError):
        temperature = 0.2
    temperature = max(0.0, min(1.0, temperature))
    strictness = divergence_meta.get("strictness")
    try:
        strictness = float(strictness) if strictness is not None else 1.0
    except (TypeError, ValueError):
        strictness = 1.0
    strictness = max(0.0, min(1.0, strictness))
    scoring_instruction = (
        "Filter to only the most grounded directions: require concrete transfer "
        "relations and reject vague analogies; scores must reflect evidence strength."
        if strictness >= 0.6
        else (
            "Prefer breadth and surprise: include more speculative cross-domain "
            "analogies and do not over-filter by evidence."
            if strictness <= 0.4
            else "Balance groundedness and breadth; keep analogies specific enough to act on."
        )
    )
    request_image_refs = (
