...
export function useStudioStore() {
  ...
  const extractGenerationArtifacts = (payload: Record<string, unknown>) => {
    const items: Array<{ url: string; kind: "png" | "glb" | "obj" }> = [];
    const push = (url: unknown, kind: "png" | "glb" | "obj") => {
      if (typeof url === "string" && url.trim()) items.push({ url, kind });
    };
    const artifacts = Array.isArray(payload?.artifacts) ? (payload.artifacts as Array<Record<string, unknown>>) : [];
    for (const artifact of artifacts) {
      const url = artifact?.url ?? artifact?.file_url ?? null;
      const kind = String(artifact?.kind ?? artifact?.type ?? "png");
      push(url, kind === "glb" ? "glb" : kind === "obj" ? "obj" : "png");
    }
    push(payload?.preview_url, "png");
    return items;
  };
  ...
  const syncFourStageGenerationArtifacts = (
    ...
    const fromRun = (run.generation_artifacts ?? []).flatMap((artifact) => {
      const url = artifact?.url ?? artifact?.file_url ?? null;
      const kind = String(artifact?.kind ?? artifact?.type ?? "png");
      return typeof url === "string" && url.trim()
        ? [{ url, kind: kind === "glb" ? ("glb" as const) : kind === "obj" ? ("obj" as const) : ("png" as const) }]
        : [];
    });
    ...
    try {
      ...
      if (versionNodeId) {
        await patchVersionNode(versionNodeId, {
          status: "mesh_ready",
          preview_url: candidatePreviewUrl(updated),
          mesh_url: updated.mesh_url,
          obj_url: updated.obj_url,
          error: null,
        });
      }
